# common__Deblurring__GatedModule_SCSE.py
...
class PreActBasicBlockDW(ResidualBase):
    def __init__(self, planes: int, stochastic_depth: bool = False,
        ...
        self.scse1 = SCSEBlock(planes, reduction, act_mode)

        self.aff2 = Affine2d(planes, affine_init_w)
        self.conv2 = conv3x3(planes, planes, groups=planes)
        self.scse2 = SCSEBlock(planes, reduction, act_mode)
        self.act = get_activation(act_mode)
...
class MBConvBlock(ResidualBase):
    def __init__(self, planes: int, stochastic_depth: bool = False, act_mode: str = 'relu',
        ...
        self.aff2 = Affine2d(planes, affine_init_w)
        self.scse = SCSEBlock(planes, reduction, act_mode)
...
# ## Adding skip connections with SCSE block. (approach 2)
class RCANBlock(ResidualBase):
    def __init__(self, planes: int, bias: bool = True, act_mode: str = 'relu',
        ...
        self.cnv3x3_1 = conv3x3(planes, planes, bias=bias)
        self.acti = get_activation(act_mode)
        self.cnv3x3_2 = conv3x3(planes, planes, bias=bias)
        self.scse = SCSEBlock(planes, reduction, act_mode)
        self.conv1x1_1 = conv1x1(192, planes)
    ...
    def _forward_res(self, x: torch.Tensor) -> torch.Tensor:
        conv_acti = self.acti(self.cnv3x3_1(x))
        conv_acti_conv = self.cnv3x3_2(conv_acti)
        out1 = torch.cat([conv_acti_conv, conv_acti, x], dim = 1)
        out2 = self.conv1x1_1(out1)
        out3 = self.scse(out2)
        out4 = out3 + out2
        out4 = out4.mul(self.res_scale)
        return out4
...
class RCANBlockDW(ResidualBase):
    ...
    def __init__(self, planes: int, bias: bool = True, act_mode: str = 'relu',
        ...
        self.body = nn.Sequential(
            ...
            SCSEBlock(planes, reduction, act_mode))
...
class MeanShift_1(nn.Conv2d):
    def __init__(self, rgb_range, rgb_mean=(0.4488,),
                 rgb_std=(1.0,), sign=-1):
        super(MeanShift_1, self).__init__(1, 1, kernel_size=1)
        std = torch.Tensor(rgb_std)
...
#class Deblur_2(nn.Module):
#    def __init__(self):
#        super(Deblur_2, self).__init__()
#
...
#            nn.ReLU(True),
#            )
#        self.final = nn.ConvTranspose2d(64, 1, kernel_size=3, stride=1, padding=1)
#            
#    def forward(self,x):  
#        x = self.encoder(x)
#        x = self.decoder(x)
...
class GateMoudle_SCSE(nn.Module):
    def __init__(self, channel, planes: int, reduction: int = 8, act_mode: str = 'relu'):
        super(GateMoudle_SCSE, self).__init__()
        self.conv1 = conv3x3(channel,  planes)
        self.scse = SCSEBlock(planes, reduction, act_mode)
    ...
    def forward(self, x):
        con1 = self.scse(self.relu(self.conv1(x)))
