#  arch__Deblurring__GatedModule_SCSE.py
...
class ISRNet(nn.Module):
    def __init__(self, n_resgroups: int, n_resblocks: int, planes: int, scale: int, 
                 prob: List[float], block_type: str, reduction: int, channels: int = 3, 
                 rgb_range: int = 255, act_mode: str = 'relu', **kwargs):
        super().__init__()
        self.sub_mean = MeanShift_1(rgb_range)
        self.add_mean = MeanShift_1(rgb_range, sign=1)
        self.scale_dim = scale
        ...
        modules_tail = [
            ...
            conv3x3(planes, planes, bias=True)]

        self.head = nn.Sequential(*modules_head)
        self.body = nn.Sequential(*modules_body)
        self.tail = nn.Sequential(*modules_tail)
        
        self.deblur = Deblur_2()
        self.gated_module_scse = GateMoudle_SCSE(129, planes, reduction, act_mode)
        
        # self.scse = SCSEBlock(planes, reduction, act_mode)
        
        self.conv1x1_tail = conv1x1(2*planes, planes)
        self.conv3x3_tail = conv3x3(planes, 1, bias=True)

    def forward(self, x):      
        init = x
        deblur_feature, deblur_out = self.deblur(x)
        
        init = self.sub_mean(x)
        init = self.head(init)
        res = self.body(init)
        res+=init
        sr_feature = res
        ...
        #SR Features
        sub_mean = self.sub_mean(x)
        head = self.head(sub_mean)
        body = self.body(head)
        sr_feature = body + init
        
        #Gated Module
        scoremap = self.gated_module_scse(torch.cat((deblur_feature, x, sr_feature), 1))
        
        repair_feature = torch.mul(scoremap, deblur_feature)
        fusion_feature = torch.add(sr_feature, repair_feature)
        
        #Reconstruction features
        reconstruct = self.tail(fusion_feature) 
             
        reconstruct = self.conv3x3_tail(reconstruct)  
       
        recon_out = self.add_mean(reconstruct)
...
class Model(nn.Module):
    ...
    def make_model(self, cfg):
        ...
        options = {
            ...
            'reduction': cfg.MODEL.SE_REDUCTION,  # SCSE block
...
#         sr_model = self._make_net(ISRNet(**options))
#         deblur_model = self._make_net(Deblur_2())
#         return deblur_model, sr_model
