...
# Including input for Low Resolution Blur
...
class SRData(data.Dataset):
    def __init__(self, cfg, name='', train=True, benchmark=False):
        ...
        self._set_filesystem(cfg.DATASET.DATA_DIR)  # -----------------------> Add BLR 
        if cfg.DATASET.DATA_EXT.find('img') < 0:
            path_bin = os.path.join(self.apath, 'bin')
            os.makedirs(path_bin, exist_ok=True)

        list_hr, list_lr, list_blr = self._scan()   # -----------------------> Add BLR      # list_blr: Blur Low Resolution
        if cfg.DATASET.DATA_EXT.find('bin') >= 0:
            # Binary files are stored in 'bin' folder
            # If the binary file exists, load it. If not, make it.
            list_hr, list_lr, list_blr = self._scan()
            self.images_hr = self._check_and_load(cfg.DATASET.DATA_EXT, list_hr, self._name_hrbin())
            
            self.images_lr = [
                self._check_and_load(cfg.DATASET.DATA_EXT, l, self._name_lrbin(s)) for s, l in zip(self.scale, list_lr)]
            
            self.images_blr = [
                self._check_and_load(cfg.DATASET.DATA_EXT, b, self._name_blrbin(s)) for s, b in zip(self.scale, list_blr)]
            
        else:
            if cfg.DATASET.DATA_EXT.find('img') >= 0 or benchmark:
                self.images_hr, self.images_lr, self.images_blr = list_hr, list_lr, list_blr
                ...
                for s in self.scale:
                    os.makedirs(
                        os.path.join(
                            self.dir_blr.replace(self.apath, path_bin),
                            'X{}'.format(s)
                        ),
                        exist_ok=True
                    )

                self.images_hr, self.images_lr, self.images_blr = [], [[] for _ in self.scale], [[] for _ in self.scale]
                for h in list_hr:
                ...
                for i, ll in enumerate(list_blr):
                    for l in ll:
                        b = l.replace(self.apath, path_bin)
                        b = b.replace(self.ext[1], '.pt')
                        self.images_blr[i].append(b)
                        self._check_and_load(
                            cfg.DATASET.DATA_EXT, [l], b,  verbose=True, load=False
                        )
    ...
    def _scan(self):
        ...
        for i in range(self.begin, self.end + 1):
            ...
            for si, s in enumerate(self.scale):
                list_blr[si].append(os.path.join(
                    self.dir_blr,
                    'X{}/{}x{}{}'.format(s, filename, s, self.ext[1])
                ))

        return list_hr, list_lr, list_blr

    def _set_filesystem(self, dir_data):
        self.apath = os.path.join(dir_data, self.name)
        self.dir_hr = os.path.join(self.apath, 'HR')
        self.dir_lr = os.path.join(self.apath, 'LR_bicubic')
        self.dir_blr = os.path.join(self.apath, 'LR_bicubic_blur')
    ...
    def _name_blrbin(self, scale):
        return os.path.join(
            self.apath,
            'bin',
            '{}_bin_BLR_X{}.pt'.format(self.split, scale)
        )
    ...
    def __getitem__(self, idx):
        blr, lr, hr, filename = self._load_file(idx)
        pair = self.get_patch(blr, lr, hr)
    ...
    def _load_file(self, idx):
        ...
        f_blr = self.images_blr[self.idx_scale][idx]
        ...
        if self.cfg.DATASET.DATA_EXT.find('bin') >= 0:
            ...
            if self.cfg.DATASET.DATA_EXT == 'img' or self.benchmark:
                ...
                blr = imageio.imread(f_blr)
            elif self.cfg.DATASET.DATA_EXT.find('sep') >= 0:
                with open(f_hr, 'rb') as _f:
                    hr = pickle.load(_f)
                with open(f_lr, 'rb') as _f:
                    lr = pickle.load(_f)
                with open(f_blr, 'rb') as _f:
                    blr = pickle.load(_f)

        return blr, lr, hr, filename

    def get_patch(self, blr, lr, hr):
        ...
        if not self.train:
            ...
            lr_patch = lr
            blr_patch = blr
            return blr_patch, lr_patch, hr_patch

        # rejection sampling for training
        while True:
            blr_patch, lr_patch, hr_patch = common.get_patch(
                blr, lr, hr, patch_size=self.cfg.DATASET.OUT_PATCH_SIZE,
            ...
            bicub_sr = resize(blr_patch, hr_patch.shape, order=3,  # bicubic
        ...
        if aug_cfg.ENABLED:
            blr_patch, lr_patch, hr_patch = common.augment(
                blr_patch, lr_patch, hr_patch, invert=aug_cfg.INVERT,
