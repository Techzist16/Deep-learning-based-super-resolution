...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.iter_start, self.iteration_total):
            ...
            blr, lr, hr, _ = next(self.loader_train)
            
            blr = blr.to(self.device, non_blocking=True)
            lr = lr.to(self.device, non_blocking=True)
            hr = hr.to(self.device, non_blocking=True)
            if hasattr(self, 'mixuper'):
                blr, lr, hr = self.mixuper(blr, lr, hr)
            ...
            with autocast(enabled=autocast_enabled):
                lr_deblur, sr = self.model(blr)
                loss1 = self.loss(lr_deblur, lr)
                loss2 = self.loss(sr, hr)
            ...
            del blr, lr, hr, sr, loss # Release some GPU memory
        ...
        if self.ckp is not None:
            ...
            for idx_data, d in enumerate(self.loader_test):
                ...
                for idx_scale, scale in enumerate(self.scale):
                    ...
                    if self.cfg.SOLVER.TEST_ONLY:
                        for blr, hr, filename in tqdm(d, ncols=80):    

                            blr = blr.to(self.device, non_blocking=True)
                            with autocast(enabled=self.cfg.MODEL.MIXED_PRECESION):
                                lr_deblur, sr = self.swa_model(blr) if is_swa else self.model(blr)
                        ...
                        for blr, lr, hr, filename in tqdm(d, ncols=80):
